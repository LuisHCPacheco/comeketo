<?php
    ...
    $senha = isset($_POST['senhaLog']) ? $_POST['senhaLog'] : '';
    $senhaCodificada = hash('sha256', $senha);

    include_once 'conexao.php';                      
    $conn = new conexao();
    
    $sql = mysqli_query($conn, "SELECT * FROM usuario WHERE email = '$email' AND senha = '$senhaCodificada'"); 
    $row = mysqli_num_rows($sql);
    $teste = mysqli_fetch_assoc($sql);

    if($row > 0) {
        $_SESSION['id_usuario'] = $teste['id'];
        $_SESSION['login'] = $teste['nome'];
        $_SESSION['id_perfil'] = $teste['id_perfil'];
        ...
        if($_SESSION['id_perfil'] == '1'){
            ...
            header("Location: http://appcomeketo.com.br/user.php");
        }
    }
    else{
        $_SESSION['msg'] = "Informações incorretas, tente novamente!";
    }
?>
<!DOCTYPE html>
<html lang="pt-br">
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-exclamation-sign"></span>  Erro ao realizar Login!</h4>
                </div>
                <div class="modal-body">								
                    <?php echo '<center><h5><b>'.$_SESSION['msg'].'</b></h5></center>'; ?>
                </div>
